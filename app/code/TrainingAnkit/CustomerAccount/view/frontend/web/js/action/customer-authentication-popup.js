define([
    ...
    $.widget('trainingankit.customerAuthenticationPopup', {
        options: {
                    ...
                    if (form.hasClass('form-create-account')) {
                        $.ajax({
                            ...
                            success: function (response) {
                                self.showResponse(response, form.find('input[name="redirect_url"]').val());
                            },
                            error: function() {
                                self.showFailingMessage();
                            }
                        });
                    } else {
                        var submitData = {},
         ...
         * @private
         */
        displayMessages: function(className, message) {
         ...
         */
        showResponse: function(response, locationHref) {
            var self = this,
                timeout = 800;
            this.element.find('.messages').html('');
            if (response.errors) {
                this.displayMessages('message-error error', response.message);
            } else {
                this.displayMessages('message-success success', response.message);
            }
         ...
         * @private
         */
        showFailingMessage: function() {
            this.element.find('.messages').html('');
            this.displayMessages('message-error error', $t('An error occurred, please try again later.'));
            this.element.find('.messages .message').show();
        }
    });

    return $.trainingankit.customerAuthenticationPopup;
});
